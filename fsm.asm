...
Inc_Done:
    ...
    mov Count1ms+0, a
    mov Count1ms+1, a
    ; Increment the BCD counter(two bytes)
    mov a, BCD_counter+0  ; Load least significant byte
    add a, #0x01          
    da a
    mov BCD_counter+0, a 
...
avg_loop:
	...
	Load_y(49300) ; VCC voltage measured
	lcall mul32
	Load_y(4095) ; 2^12-1
	...
	Load_y(49300) ; VCC voltage measuredm
	lcall mul32 
 ...
 ; --- Accumulate this sample into Avg_Sum ---
    ...
    mov  x+2, Avg_Sum+2
    mov  x+3, Avg_Sum+3
    Load_y(10)      ; divide by the average amount
...
ret
jumper2:
ljmp Display_Ramp_To_Soak
ret
jumper3:
ljmp Display_Ramp_To_Peak
ret
jumper4:
ljmp Display_Preheat_Soak
ret
jumper5:
ljmp Display_Reflow
ret
jumper6:
ljmp Display_Cooling
ret

; Function to display "Idle"
Display_Idle:
    mov A, #'I'
    lcall putchar
    mov A, #'d'
...
; Function to display "Ramp To Soak"
Display_Ramp_To_Soak:
    mov A, #'R'
    lcall putchar
    mov A, #'a'
    lcall putchar
...
; Function to display "Preheat/Soak"
Display_Preheat_Soak:
    mov A, #'P'
    lcall putchar
    mov A, #'r'
    lcall putchar
...
; Function to display "Ramp To Peak"
Display_Ramp_To_Peak:
    mov A, #'R'
    lcall putchar
    mov A, #'a'
    lcall putchar
...
state0: ; Start button dependent
    ...
    setb s0 ;this tells us what state we are in
    clr s1
    clr s2
    clr s3
    clr s4
    clr s5
    setb ison ;turn the buzzer on then off
...
state1_loop:
    ...
    ; check for abort
    ; check higher byte BCD_counter
    mov a, BCD_counter+1
    clr c
    subb a, #0
    jnz check_temp_soak_begin
    ; check for lower byte of BCD_counter
    mov a, BCD_counter+0
    clr c
    subb a, #96 ;60 second wait 
    jnc check_temp_50_upper
    sjmp check_temp_soak_begin
check_temp_50_upper:   ; supposed to abort if requirement not is met
...
check_temp_soak:
    ...
    jb abort_flag, s1_jump_state0
    lcall LCD_PB; get the pushbuttons value
	jb PB0, state1_loop
PB0_s1_check_release:
	lcall LCD_PB
	jnb PB0, PB0_s1_check_release  
    sjmp jump_state0
s1_jump_state0:
    ljmp jump_state0
...
check_time_soak:
    ...
    lcall LCD_PB; get the pushbuttons value
	jb PB0, state2_loop
PB0_s2_check_release:
	lcall LCD_PB
	jnb PB0, PB0_s2_check_release  
	mov R2, #50
	lcall waitms    
    sjmp jump_state0
...
check_temp_refl:
    ...
    lcall LCD_PB; get the pushbuttons value
	jb PB0, state3_loop
PB0_s3_check_release:
	lcall LCD_PB
	jnb PB0, PB0_s3_check_release  
	mov R2, #50
	lcall waitms        
    sjmp jump_state0
...
check_time_refl:
    ...
    lcall LCD_PB; get the pushbuttons value
	jb PB0, state4_loop
PB0_s4_check_release:
	lcall LCD_PB
	jnb PB0, PB0_s4_check_release  
	mov R2, #50
	lcall waitms        
    ljmp jump_state0
...
check_state5:
    ...
    lcall LCD_PB; get the pushbuttons value
	jb PB0, state5_loop
    PB0_s5_check_release:
    lcall LCD_PB
    jnb PB0, PB0_s5_check_release  
    mov R2, #50
    lcall waitms        
    ljmp jump_state0
